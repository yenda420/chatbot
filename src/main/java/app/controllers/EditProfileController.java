...
public class EditProfileController {
    ...
    public void onSave() throws SQLException {
        ...
        if (validateInputs()) {
            ...
            if (editingCurrentUser) {
                isTeacher = UserRoleEnum.fromString(role.getValue()).equals(UserRoleEnum.TEACHER);
            }

            if (wasAdmin && isTeacher) {
                boolean adminConfirmedChanges = AlertService.showConfirmationAlert(
                        "Potvrzení",
                        "Moment!",
                        "Opravdu chcete změnit vaši roli z administrátora na učitele?");

                if (adminConfirmedChanges) {
                    executeUpdate(user, oldEmail, emailChanged, true);
                }
            } else {
                executeUpdate(user, oldEmail, emailChanged, false);
            }
        }
    }

    private void executeUpdate(User user, String oldEmail, boolean emailChanged, boolean redirectToLogin) {
