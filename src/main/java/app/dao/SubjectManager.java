...
public class SubjectManager {
    ...
    public static boolean update(Subject oldSubject, Subject newSubject) throws SQLException {
        if (db.getConn() != null) {
            String sql;

            if (newSubject.getDescription() != null) {
                sql = "UPDATE subjects SET name = ?, abbreviation = ?, description = ? WHERE name = ?";
            } else {
                sql = "UPDATE subjects SET name = ?, abbreviation = ? WHERE name = ?";
            }

            try (PreparedStatement pstmt = db.getConn().prepareStatement(sql)) {
                pstmt.setString(1, newSubject.getName());
                pstmt.setString(2, newSubject.getAbbreviation());

                if (newSubject.getDescription() != null) {
                    pstmt.setString(3, newSubject.getDescription());
                    pstmt.setString(4, oldSubject.getName());
                } else {
    ...
    public static int getId(Subject subject) throws SQLException {
    ...
    public static int getId(int topicId) throws SQLException {
        String sql = "SELECT subjectId FROM topics WHERE topicId = ?";
        try (PreparedStatement pstmt = db.getConn().prepareStatement(sql)) {
